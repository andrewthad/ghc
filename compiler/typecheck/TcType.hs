...
tcTyFamInstsAndVisX = go
  where
    ...
    go is_invis_arg (ForAllTy bndr ty) = go is_invis_arg (binderType bndr)
                                         ++ go is_invis_arg ty
    go is_invis_arg (FunTy m ty1 ty2)  = go is_invis_arg m
                                         ++ go is_invis_arg ty1
                                         ++ go is_invis_arg ty2
...
exactTyCoVarsOfType ty
  ...
  where
    ...
    go (FunTy m arg res)    = go m `unionVarSet` go arg `unionVarSet` go res
...
anyRewritableTyVar ignore_cos role pred ty
  ...
  where
    ...
    go rl bvs (FunTy m arg res) = go NomEq bvs m || go rl bvs arg || go rl bvs res
...
tcSplitPiTys ty = ASSERT( all (isTyBinder . snd) (fst sty) ) sty
...
tcSplitPredFunTy_maybe (FunTy m arg res)
...
tcSplitFunTy_maybe (FunTy m arg res) | not (isPredTy arg) = Just (m, arg, res)
...
tc_eq_type view_fun orig_ty1 orig_ty2 = go True orig_env orig_ty1 orig_ty2
  where
    ...
    go vis env (FunTy m1 arg1 res1) (FunTy m2 arg2 res2)
      = go vis env m1 m2 <!> go vis env arg1 arg2 <!> go vis env res1 res2
    go vis env ty (FunTy m arg res)
      = eqFunTy vis env m arg res ty
    go vis env (FunTy m arg res) ty
      = eqFunTy vis env m arg res ty
    ...
    -- corner case.
    eqFunTy :: Bool -> RnEnv2 -> Matchability -> Type -> Type -> Type -> Maybe Bool
    eqFunTy vis env m arg res (FunTy m' arg' res')
      = go vis env m m' <!> go vis env arg arg' <!> go vis env res res'
    eqFunTy vis env m arg res ty@(AppTy{})
      | Just (tc, [m', _, _, arg', res']) <- get_args ty []
      , tc == funTyCon
      = go vis env m m' <!> go vis env arg arg' <!> go vis env res res'
      where
...
isInsolubleOccursCheck eq_rel tv ty
  ...
  where
    ...
    go (AppTy t1 t2) = case eq_rel of  -- See Note [AppTy occurs checking]
                         ...
                         ReprEq -> go t1
    go (FunTy m t1 t2) = go m || go t1 || go t2
...
isRhoTy (FunTy m a r) = isRhoTy m && not (isPredTy a) && isRhoTy r
isRhoTy _             = True
...
sizeType = go
  where
    ...
    go (LitTy {})                = 1
    go (FunTy m arg res)         = go m + go arg + go res + 1
