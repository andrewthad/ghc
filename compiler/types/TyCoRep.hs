...
module TyCoRep (
        ...
        mkFunTy, mkFunTyU, mkFunTyM, mkFunTys, mkFunTysU, mkFunTysM, mkTyCoForAllTy, mkForAllTys,
        mkForAllTy,
        ...
        isMatchabilityTy, isMatchabilityVar,
        isMatchableTy, isUnmatchableTy,
        sameVis,
...
import {-# SOURCE #-} TysWiredIn ( unmatchableDataConTy, matchableDataConTy )
...
data Type
  ...
  | FunTy Matchability Type Type
                        -- ^ t1 ->{m} t2   Very common, so an important special case
...
-- | Make an arrow type with a given matchability
mkFunTy :: Matchability -> Type -> Type -> Type
mkFunTy m arg res = FunTy m arg res

infixr 3 `mkFunTyM`      -- Associates to the right
-- | Make a matchable arrow type
mkFunTyM :: Type -> Type -> Type
mkFunTyM arg res = FunTy matchableDataConTy arg res

infixr 3 `mkFunTyU`      -- Associates to the right
-- | Make an unmatchable arrow type
mkFunTyU :: Type -> Type -> Type
mkFunTyU arg res = FunTy unmatchableDataConTy arg res

-- | Make nested arrow types
mkFunTys :: [(Matchability, Type)] -> Type -> Type
mkFunTys tys ty = foldr (uncurry mkFunTy) ty tys

-- | Make nested matchable arrow types
mkFunTysM :: [Type] -> Type -> Type
mkFunTysM tys ty = foldr mkFunTyM ty tys

-- | Make nested unmatchable arrow types
mkFunTysU :: [Type] -> Type -> Type
mkFunTysU tys ty = foldr mkFunTyU ty tys
...
mkTyCoPiTy :: Matchability -> TyCoBinder -> Type -> Type
mkTyCoPiTy m (Anon ty1) ty2           = FunTy m ty1 ty2
mkTyCoPiTy m (Named (Bndr tv vis)) ty = mkTyCoForAllTy tv vis ty

-- | Like 'mkTyCoPiTy', but does not check the occurrence of the binder
mkPiTy:: Matchability -> TyCoBinder -> Type -> Type
mkPiTy m (Anon ty1) ty2           = FunTy m ty1 ty2
mkPiTy m (Named (Bndr tv vis)) ty = mkForAllTy tv vis ty

mkTyCoPiTys :: [(Matchability, TyCoBinder)] -> Type -> Type
mkTyCoPiTys tbs ty = foldr (uncurry mkTyCoPiTy) ty tbs

-- | Like 'mkTyCoPiTys', but does not check the occurrence of the binder
mkPiTys :: [(Matchability, TyCoBinder)] -> Type -> Type
mkPiTys tbs ty = foldr (uncurry mkPiTy) ty tbs
...
-- | Is this the type 'Matchability'?
isMatchabilityTy :: Type -> Bool
isMatchabilityTy ty | Just ty' <- coreView ty = isMatchabilityTy ty'
isMatchabilityTy (TyConApp tc []) = tc `hasKey` matchabilityTyConKey
isMatchabilityTy _ = False

-- | Is a tyvar of type 'Matchability'?
isMatchabilityVar :: TyVar -> Bool
isMatchabilityVar = isMatchabilityTy . tyVarKind

isMatchableTy, isUnmatchableTy :: Type -> Bool
isMatchableTy ty
  | Just ty' <- coreView ty          = isMatchableTy ty'
  | TyConApp m_tc args <- ty
  , m_tc `hasKey` matchableDataConKey = ASSERT( null args ) True
  | otherwise                         = False
isUnmatchableTy ty
  | Just ty' <- coreView ty          = isUnmatchableTy ty'
  | TyConApp m_tc args <- ty
  , m_tc `hasKey` unmatchableDataConKey = ASSERT( null args ) True
  | otherwise                         = False
...
data Coercion
  ...
  | FunCo Role KindCoercion Coercion Coercion         -- lift FunTy
         -- FunCo :: "e" -> N -> e -> e -> e
...
ty_co_vars_of_type (FunTy m arg res)  is acc = ty_co_vars_of_type m is (ty_co_vars_of_type arg is (ty_co_vars_of_type res is acc))
...
ty_co_vars_of_co (FunCo _ mco co1 co2)is acc = ty_co_vars_of_co mco is $
...
tyCoFVsOfType (FunTy m arg res)  f bound_vars acc = (tyCoFVsOfType m `unionFV` tyCoFVsOfType arg `unionFV` tyCoFVsOfType res) f bound_vars acc
...
tyCoFVsOfCo (FunCo _ mco co1 co2)    fv_cand in_scope acc
  = (tyCoFVsOfCo mco `unionFV` tyCoFVsOfCo co1 `unionFV` tyCoFVsOfCo co2) fv_cand in_scope acc
...
almost_devoid_co_var_of_co (FunCo _ mco co1 co2) cv
  = almost_devoid_co_var_of_co mco cv
...
almost_devoid_co_var_of_type (FunTy m arg res) cv
...
injectiveVarsOfType = go
  where
    ...
    go (FunTy m ty1 ty2) = go m `unionFV` go ty1 `unionFV` go ty2
    go (TyConApp tc tys) =
...
noFreeVarsOfType (FunTy m t1 t2)  = noFreeVarsOfType m && noFreeVarsOfType t1 && noFreeVarsOfType t2
...
noFreeVarsOfCo (FunCo _ mco c1 c2)    = noFreeVarsOfCo mco && noFreeVarsOfCo c1 && noFreeVarsOfCo c2
...
subst_co subst co
  ...
  where
    ...
    go (FunCo r mco co1 co2) = ((mkFunCo r $! go mco) $! go co1) $! go co2
...
debug_ppr_ty prec (FunTy m arg res)
  = maybeParen prec funPrec $
    sep [debug_ppr_ty funPrec arg, arr <+> debug_ppr_ty prec res]
  where arr = arrow <> char '{' <> debug_ppr_ty prec m <> char '}'
...
tidyType env (FunTy m fun arg)    = ((FunTy $! (tidyType env m)) $! (tidyType env fun)) $! (tidyType env arg)
...
tidyCo env@(_, subst) co
  ...
  where
    ...
    go (FunCo r mco co1 co2) = ((FunCo r $! go mco) $! go co1) $! go co2
...
typeSize (FunTy m t1 t2)            = typeSize m + typeSize t1 + typeSize t2
...
coercionSize (FunCo _ mco co1 co2) = 1 + coercionSize mco + coercionSize co1 + coercionSize co2
